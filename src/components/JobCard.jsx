...
export default function JobCard({ job, onSaveToggle }) {
  ...
  return (
    <div onClick={() => navigate(`/job/${job.id}`)} className="cursor-pointer group relative rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-lg"
      style={{ background: '#fff', border: isFeatured ? '1.5px solid var(--gold)' : '1px solid rgba(4,50,34,0.08)', boxShadow: isFeatured ? '0 4px 20px rgba(201,168,76,0.12)' : '' }}>
      ...
      {isNew && !isFeatured && (
        <div className="absolute -top-px right-4 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-b-lg"
          style={{ background: 'var(--gold)', color: 'var(--forest)' }}>New</div>
      )}
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-3">
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            style={{ border: `1px solid ${isSaved ? 'var(--gold)' : 'rgba(4,50,34,0.12)'}`, background: isSaved ? '#fffbf0' : 'transparent', color: isSaved ? 'var(--gold)' : 'var(--muted)' }}>
        ...
        <div className="flex items-center justify-between pt-3" style={{ borderTop: '1px solid rgba(4,50,34,0.07)' }}>
          ...
          <button className="btn-primary text-xs px-4 py-2" onClick={e => { e.stopPropagation(); navigate(`/job/${job.id}`) }}>
            View →
          </button>
        </div>
      </div>
